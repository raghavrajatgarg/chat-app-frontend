...
export default function ChatFeed({
  ...
  // Track button references and screen coordinates for Portal dropdown positioning
  ...
  // Toggle menu and calculate coordinates for Portal placement (aligning right edges)
  const handleToggleMenu = (e, messageId) => {
    e.stopPropagation();
    const isMenuOpen = openMenuId === messageId;

    if (!isMenuOpen && buttonRefs.current[messageId]) {
      const rect = buttonRefs.current[messageId].getBoundingClientRect();
      const dropdownWidth = 160; // Matches CSS width

      setDropdownCoords({
        top: rect.bottom + window.scrollY + 4,
        left: rect.right + window.scrollX - dropdownWidth, // Keeps it neatly tucked under the button
      });
                    ...
                    <div className={`${styles.messageBubbleBase} ${isMe ? styles.messageBubbleMe : styles.messageBubbleOther}`}>
                      ...
                      {isMe && msg._id && (
                        <div className={`${styles.messageActionTrigger} ${isMenuOpen ? styles.forceVisible : ''}`}>
                          <button 
                            ref={(el) => (buttonRefs.current[msg._id] = el)}
